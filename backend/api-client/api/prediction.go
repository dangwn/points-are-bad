 ...
 */
func getPredictionsByUserId(userId string, startDate, endDate *Date) ([]PredictionWithMatch, error) {
    var predictions []PredictionWithMatch

    predictionsQuery := `
        WITH uap AS (
            SELECT prediction_id, home_goals, away_goals, match_id
            FROM predictions
            WHERE predictions.user_id = $1
        )
        SELECT prediction_id, uap.home_goals, uap.away_goals, matches.match_id, match_date,
...
func populatePredictionsByUserId(userId string) error {
    ...
    if n := len(matchIds); n == 0 {
        return nil
    } else {
        predictionIds = createUUIDSlice(n)
    }

    // Unnests match Ids into query string (see customTypes.go)
    insertQuery := `
        INSERT INTO predictions(home_goals, away_goals, user_id, match_id, prediction_id)
        SELECT NULL, NULL, $1, ` + UnnestArray(matchIds).String() + ", " + UnnestArray(predictionIds).String()
...
func populatePredictionsByMatchId(matchId string) error {
    ...
    insertQuery := `
        INSERT INTO predictions(home_goals, away_goals, match_id, user_id, prediction_id)
        SELECT NULL, NULL, $1, ` + UnnestArray(userIds).String() + ", " + UnnestArray(predictionIds).String()
